...
> **Role: Team Lead**  
> 회의 진행, 일정 관리, 역할 분담, 주간 Sprint 운영, GitHub 통합 확인, 지연 작업 조정, 공모전 필수 범위 점검을 담당했습니다.

### Leadership Goal

`fair-decision-rag`는 공모전의 목표와 제출 조건이 정해진 프로젝트였습니다. 팀은 새로운 기술인 RAG를 직접 구현하는 데 도전했고, **공모전에서 요구한 조건을 충족하는 MVP를 완성해 제출하는 것**을 공동 목표로 정했습니다.

모든 팀원이 새로운 기술을 개발하는 과정에 흥미를 가지고 있었기 때문에, 단순히 문서 작성이나 보조 업무를 나누기보다 **실질적인 기술 단위를 분리해 각자 구현하는 방식**으로 역할을 배분했습니다.

제가 회의를 진행할 때 가장 중요하게 확인한 항목은 다음과 같습니다.

- 현재 프로젝트가 어느 단계까지 진행되었는가
- 담당 기능을 구현하면서 팀원이 제기한 문제는 없는가
- 각 코드가 전체 Pipeline 안에서 원활하게 연결되고 있는가
- 현재 개발 방향이 공모전의 목표와 제출 조건에 맞는가
- 남은 일정에서 무엇을 우선 구현하고 무엇을 조정해야 하는가

### Meeting and Development Process

| Meeting Stage | Main Agenda | Leadership Work |
|---|---|---|
| **1차 회의** | 구현할 프로젝트의 범위와 사용할 기술 결정 | 공모전의 궁극적인 목표를 기준으로 RAG MVP 범위와 필요한 기술을 정리 |
| **2차 회의** | 역할과 담당 Part 결정 | 각자가 담당할 실질적인 기술 영역과 구현 Part를 분리하고 역할 분담 |
| **3차 회의** | 개발 Process와 일정 확정 | 어느 수준까지 구현할지, 어떤 순서로 개발할지, 각 작업의 완료 시점을 정하고 본격적인 구현 시작 |
| **주간 정비 회의** | 진행 상황, 방향성, 문제, 개선 사항 점검 | 회의 전에 각 팀원에게 이번 주 수행 작업을 요청하고, 회의에서 진행 단계와 개선 방향을 검토 |
| **통합 점검** | 분리된 코드가 하나의 Project로 연결되는지 확인 | GitHub를 통해 각 Part의 코드와 통합 상태를 확인하고, 연결되지 않은 부분과 다음 작업을 정리 |

### Weekly Sprint and Schedule Coordination

팀원 모두 학교 일정과 개인 일정이 달랐기 때문에 고정된 시간만으로 회의를 운영하기 어려웠습니다. 특히 한 팀원은 개인 Schedule로 인해 일정 변동이 잦았습니다.

이를 해결하기 위해 다음 방식으로 운영했습니다.

1. **TimePick으로 공통 가능 시간 확인**
   - 모든 팀원이 가능한 시간을 먼저 파악했습니다.
   - 기본 회의일은 토요일로 정했습니다.
   - 학교와 개인 일정에 따라 회의 시간을 유동적으로 조정했습니다.

2. **Notion 주간 Sprint 운영**
   - 일주일 단위로 구현해야 할 작업을 Notion에 기록했습니다.
   - 현재 어느 기능을 구현해야 하는지 명확하게 확인할 수 있도록 했습니다.
   - 역할별 Code Part와 완료해야 하는 시점을 함께 지정했습니다.

3. **회의록 기록**
   - 회의에서 결정한 내용과 다음 작업을 Notion 회의록에 남겼습니다.
   - 다음 회의에서 빠뜨리는 항목이 없도록 이전 결정 사항과 진행 상태를 다시 확인했습니다.

4. **GitHub 통합 확인**
   - 주간 회의에서 각자가 작성한 코드가 저장소에 반영되었는지 확인했습니다.
   - 분리된 기능이 전체 Retrieval, RAG Pipeline 안에서 원활하게 연결되는지 점검했습니다.

### Delay Management and Mutual Support

프로젝트를 시작할 때부터 저는 팀원들에게 시간이 부족하거나 작업에 차질이 생기면 **혼자 지연시키지 말고 다른 팀원에게 반드시 도움을 요청해 달라**고 전달했습니다.

한 사람이 해당 시점에 작업을 완료하기 어려운 경우에는 다른 사람이 일부 Part를 맡아 전체 일정의 지연을 줄이고, 이후 여유가 생겼을 때 서로 다른 작업을 도우며 균형을 맞출 수 있다고 판단했기 때문입니다.

이 원칙에 따라 일정 변동이 많았던 팀원은 일주일 동안 개인 일정이 바쁘다는 사실을 제게 솔직하게 공유했습니다. 저는 지연된 작업을 빠르게 해소하기 위해 해당 팀원이 맡았던 Code Part의 일부를 가져와 직접 작성하고 통합을 지원했습니다.

이 과정에서 중요하게 본 것은 단순히 담당자를 바꾸는 것이 아니라 다음을 유지하는 것이었습니다.

- 기존 담당 범위를 명확하게 구분
- 추가 지원이 필요한 Code Part만 재분배
- 새 완료 시점 지정
- 전체 MVP 일정에 미치는 영향 최소화
- 이후 일정에서 서로 도움을 주고받을 수 있는 협업 방식 유지

### MVP Scope Control

일정이 변경되어 확장 기능을 모두 구현하기 어렵다고 판단되는 경우에는 **공모전에서 필수적으로 요구하는 조건과 MVP 핵심 기능이 빠지지 않도록 범위를 다시 확인**했습니다.

우선순위는 다음과 같이 관리했습니다.

```text
공모전 필수 조건
→ 실행 가능한 Retrieval Pipeline
→ 중복 없는 Top-5 chunk_id
→ 원본 ID 무결성
→ 근거 기반 Answer와 Evidence
→ 제한 시간과 Offline 실행
→ 추가 성능 개선과 확장 기능
```

확장된 범위를 모두 구현하는 것보다, 제출 가능한 상태에서 필수 조건을 빠짐없이 반영하는 것을 우선했습니다. 새로운 기능을 추가할 때도 현재 일정과 통합 상태를 확인하고, MVP 제출에 영향을 주지 않는 범위에서 진행하도록 조정했습니다.

### Leadership Outcomes

- 공모전 목표를 기준으로 MVP 범위와 개발 우선순위 정리
- 기술 단위 중심의 역할 분담과 담당 Code Part 명확화
- 1차, 2차, 3차 회의를 통해 범위, 역할, Process, 일정 확정
- 매주 작업 내용을 사전에 수집하고 정비 회의 운영
- TimePick을 활용한 공통 일정 파악과 토요일 중심의 유동적 회의 운영
- Notion 주간 Sprint, 완료 시점, 회의록 관리
- GitHub를 통한 기능별 진행 상태와 Code 통합 확인
- 문제 발생 시 조기 공유와 도움 요청 원칙 수립
- 일정 지연 Part의 일부를 직접 구현해 통합 일정 지원
- 일정 변경 시 확장 범위보다 공모전 필수 조건과 MVP 완성도 우선
